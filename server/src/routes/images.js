...
const fileSend = require("koa-send"); // 文件下载
    ...
    if (err) {
        ...
        if (stat.isDirectory()) {
            ...
            for (let fileName of files) {
                let [err, stat] = await to(
                    statAsync(imagesPath + pathName + "/" + fileName)
                );
                if (err) {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        let [err, file] = await to(
                            readFileAsync(imagesPath + pathName + "/" + fileName)
                        );
                        if (err) {
...
// 下载图片
router.get("/images/download", async ctx => {
    const { pathName } = ctx.query;
    const url = imagesPath + pathName;
    const fileName = url.split("/").pop();
    ctx.attachment(fileName);
    await fileSend(ctx, pathName, { root: imagesPath });
});
